...
export const getStatusLocation = (distance?: number, radius = 500) => {
    if(!distance) return { status: "nearBy",Icon: AwayIcon, label: "-- away" ,title: "-- away"}
    if (distance - radius <= 0) {
      ...
      if (distance - radius > 0 && distance - radius < 50000) {
          return {
                status: "nearBy",
                Icon: AwayIcon,
              label: `${Number((distance - radius).toFixed(2)) < 1000 ? `${(distance - radius).toFixed(2)}m` : `${((distance - radius) / 1000).toFixed(2)}km`} away`,
              title:`${Number((distance - radius).toFixed(2)) < 1000 ? `${(distance - radius).toFixed(2)}m` : `${((distance - radius) / 1000).toFixed(2)}km`} away`
        };
      } else {
